...
class WC_TechnoTan_Shipping extends WC_Shipping_Method {
    ...
    public function __construct( $instance_id = 0 ){
        ...
        if(WOOTAN_DEBUG) $this->wootan->debug("WC_TechnoTan_Shipping->__construct: \$this->id = ".serialize($this->id));
    ...
    public function init_form_fields() {
        ...
        $this->form_fields = array(
            ...
            'containers' => array(
                ...
                'desc_tip' => true,
                'default' =>
                    '{"AIRBAG1":{"max_weight":1.1, "max_dim":[70, 165, 260]}, '
                    .'"AIRBAG3":{"max_weight":2.7, "max_dim":[150, 160, 260]}, '
                    .'"AIRBAG5":{"max_weight":4.5, "max_dim":[250, 170, 310]}, '
                    .'"AIRBAG6":{"max_weight":5.5, "max_dim":[270, 275, 320]}, '
                    .'"LABEL1":{"max_weight":1.1}, '
                    .'"LABEL3":{"max_weight":2.7}, '
                    .'"LABEL5":{"max_weight":4.5}, '
                    .'"LABEL10":{"max_weight":9.5}, '
                    .'"LABEL20":{"max_weight":19.2}'
                    .'}'
            )
        );
    }
    ...
    public function get_container_options(){
        ...
        foreach ($this->get_containers() as $container => $details) {
            $container_name = __("max weight: ") . number_format_i18n($details['max_weight']) . $this->weight_unit;
            if(isset($details['max_dim'])){
                $container_name .= __("; max dims: ") . implode("x", $details['max_dim']) . $this->dimension_unit;
            }
    ...
    public function get_containers() {
        ...
        foreach ($containers as $key => $container) {
            if(! isset($container['max_weight'])){
                $this->errors[] = "invalid container: $key. No max_weight set.";
                unset($containers[$key]);
                continue;
            }
            if(! isset($container['max_dim'])){
                $weight = floatval($container['max_weight']);
    ...
    /**
     * Get Volume of product in cubic meters
     */
    public function get_volume_m3($dimensions){
    ...
    public function get_summary($contents){
        ...
        foreach($contents as $line){
            ...
            if($line['data']->has_dimensions()){
                ...
                $item_vol = $this->get_volume_m3($item_dim);
    ...
    function calculate_shipping( $package=array() ) {
        ...
        if( ! empty( array_filter( array_values($container_limits))) ) {
            ...
            if( ! empty($container_limits['max_item']) ) {
                ...
                foreach ($package['contents'] as $line) {
                    ...
                    if(WOOTAN_DEBUG) $this->wootan->debug("---> analysing line: ".$line['product_id']);
                    if(WOOTAN_DEBUG) $this->wootan->debug("----> line virtual: ".$line['date']->is_virtual());
                    # TODO: Ignore line if virtual
                    ...
                    if($line['data']->has_weight()){
                        ...
                        if(WOOTAN_DEBUG) $this->wootan->debug("---> failed max_item_container criteria - can't get line weight: ".serialize($line['data']));
                        $this->errors[] = "can't get weigh of ". $line['product_id'];
                        return;
                    }
                    if($line['data']->has_dimensions()){
                        $item_dim = explode(' x ', $line['data']->get_dimensions());
                        $item_dim[2] = str_replace( ' '.$this->dimension_unit, '', $item_dim[2]);
                    } else {
                        if(WOOTAN_DEBUG) $this->wootan->debug("---> failed max_item_container criteria - can't get line dimensions: ".serialize($line['data']));
                        $this->errors[] = "can't get dimensions of ". $line['product_id'];
                        return;
                    }
                    $item = array(
                        'weight' => $item_weight,
                    ...
                    if(in_array($container, array_keys($wootan_containers))){
                        ...
                        if($result){
                            if(WOOTAN_DEBUG) $this->wootan->debug("---> passed max_item_container criteria - fits in container ".$result);
                        } else {
                            if(WOOTAN_DEBUG) $this->wootan->debug("---> failed max_item_container criteria - fits in container: ".$result);
                            return;
                        }
                    } else {
        ...
        $this->add_rate( array(
            'label' => $this->title,
            'package' => $package,
            'cost' => $this->get_option('cost_base'),
        ) );
    }
}
