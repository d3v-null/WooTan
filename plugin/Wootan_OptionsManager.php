...
class Wootan_OptionsManager extends Wootan_Debugger{
    ...
    public function settingsPage() {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'wootan'));
        }
        ...
        ?>
        <div class="wrap">
            <h2><?php _e('System Settings', 'wootan'); ?></h2>
            <table class="form-table"><tbody>
            <tr><td><?php _e('System', 'wootan'); ?></td><td><?php echo php_uname(); ?></td></tr>
            <tr><td><?php _e('PHP Version', 'wootan'); ?></td>
                <td><?php echo phpversion(); ?>
                <?php
                if (version_compare('5.2', phpversion()) > 0) {
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    _e('(WARNING: This plugin may not work properly with versions earlier than PHP 5.2)', 'wootan');
                    echo '</span>';
                }
                ?>
                </td>
            </tr>
            <tr><td><?php _e('MySQL Version', 'wootan'); ?></td>
                <td><?php echo $this->getMySqlVersion() ?>
                    <?php
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    if (version_compare('5.0', $this->getMySqlVersion()) > 0) {
                        _e('(WARNING: This plugin may not work properly with versions earlier than MySQL 5.0)', 'wootan');
                    }
                    echo '</span>';
                    ?>
                </td>
            ...
            <h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'wootan'); ?></h2>
            ...
            <?php settings_fields($settingsGroup); ?>
                ...
                <p class="submit">
                    <input type="submit" class="button-primary"
                           value="<?php _e('Save Changes', 'wootan') ?>"/>
                </p>
            </form>
        </div>
        <?php
    ...
    protected function getOptionValueI18nString($optionValue) {
        switch ($optionValue) {
            case 'true':
                return __('true', 'wootan');
            case 'false':
                return __('false', 'wootan');

            case 'Administrator':
                return __('Administrator', 'wootan');
            case 'Editor':
                return __('Editor', 'wootan');
            case 'Author':
                return __('Author', 'wootan');
            case 'Contributor':
                return __('Contributor', 'wootan');
            case 'Subscriber':
                return __('Subscriber', 'wootan');
            case 'Anyone':
                return __('Anyone', 'wootan');
        }
        return $optionValue;
    }
